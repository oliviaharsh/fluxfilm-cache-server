...
| 📺 Netflix household / travel / emailed code (4-digit sign-in **or** 6-digit verify) from the ffnetflixhub inbox | `adminnetflix.js` + `oliviahousehold.js` (IMAP, one Gmail label per account) | read-only; the code is admin-only and never logged |
