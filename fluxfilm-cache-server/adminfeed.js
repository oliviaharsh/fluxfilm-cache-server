...
function mount(app, deps) {
  ...
  route('/admin/api/feed/video/settings', async (req, res, b) => {
    if (!videos) return res.status(400).json({ ok: false, message: 'Video upload is not available.' });
    const r = await videos.saveLimits({ videoMaxMb: b.videoMaxMb, videoTotalMb: b.videoTotalMb, videoMaxMbR2: b.videoMaxMbR2, videoTotalMbR2: b.videoTotalMbR2 });
    if (r.ok) audit.record(req, { action: 'feed.video.settings', entity: 'feed', id: 'videos', summary: 'Reel video limits: database ' + r.limits.videoMaxMb + ' MB per video / ' + r.limits.videoTotalMb + ' MB total · R2 ' + r.limits.videoMaxMbR2 + ' MB per video / ' + r.limits.videoTotalMbR2 + ' MB total' });
    res.status(r.ok ? 200 : 400).json(r);
  });
