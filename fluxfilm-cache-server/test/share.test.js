...
(async () => {
  ...
  ok('feed post passes the catalog info (cheapest plan) even when the post has no buy button', /shareInfo: info \|\| feedServiceInfo_\(p\.ctaService \|\| p\.service, plans\)/.test(store) && /onClick: \(\) => feedShare_\(p, shareInfo, how => \{/.test(store));
