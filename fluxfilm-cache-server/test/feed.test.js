...
(async () => {
  ...
  const V = (win, doc) => new Function('window', 'document', 'setTimeout', 'clearTimeout', vidSrc + '; return { feedYouTubeId_, feedTrailerSrc_, feedIgUrl_, feedVideoStart_, feedVideoEnd_, feedIgScript_, feedIgMount_, feedIgJs_, feedVideo_ };')(win, doc, (f) => { win._timers.push(f); return win._timers.length; }, () => {});
  ...
  ok('Instagram embed.js is not in the page: no <script src> for it, only the tap-time loader', (html.match(/['"]https:\/\/www\.instagram\.com\/embed\.js['"]/g) || []).length === 1 && vidSrc.includes("sc.src = 'https://www.instagram.com/embed.js'") && !/<script[^>]+instagram/i.test(html) && !/<script[^>]+youtube/i.test(html) && !/<iframe/i.test(html));
  ok('nothing from Instagram is added before a tap', doc.appended.length === 0 && doc.created.length === 0);
  let igOk = [];
  vid.feedIgScript_((x) => igOk.push(x)); vid.feedIgScript_((x) => igOk.push(x));
  ok('embed.js added once (async) even when two posts are tapped quickly', doc.appended.length === 1 && doc.appended[0].async === true && doc.appended[0].src === 'https://www.instagram.com/embed.js' && igOk.length === 0);
  let processed = 0; win.instgrm = { Embeds: { process: () => processed++ } };
  doc.appended[0].onload();
  ok('after load both waiting posts are told', igOk.join() === 'true,true');
  vid.feedIgScript_((x) => igOk.push(x));
  ok('already loaded → not added again', doc.appended.length === 1 && igOk.length === 3);
  const box = doc.createElement('div');
  vid.feedIgMount_(box, 'https://instagram.com/reel/C9xYz_12-ab/?igsh=1', 500);
  ...
  win._timers[0]();
  ok('embed rendered with a height → kept (no fallback)', box5.children[0] && box5.children[0].tagName === 'BLOCKQUOTE');
  // One video at a time.
  const stops = [];
  ...
  ok('tap → youtube-nocookie iframe (autoplay, fullscreen, not lazy — run 1) + close button + play event', yf && yf.props.src === vid.feedTrailerSrc_('dQw4w9WgXcQ') && yf.props.allow === 'autoplay; encrypted-media; picture-in-picture; fullscreen' && yf.props.allowFullScreen === true && yf.props.loading === undefined && find(tree, (n) => n.type === 'img' && n.props.className !== 'ff-feed-vposter').length === 0 && fm.events.join() === 'play:fp1' && /ff-feed-media vid/.test(tree.props.className));
  find(tree, (n) => n.type === 'button')[0].props.onClick();
  ...
  tree = fm.draw();
  ok('reel without a picture: branded card "📸 Watch on Instagram" + small trailer button; nothing embedded yet', /📸 Watch on Instagram/.test(text(tree)) && /ig/.test(find(tree, (n) => /ff-feed-noimg/.test(n.props.className || ''))[0].props.className) && find(tree, (n) => n.type === 'button').length === 2 && find(tree, (n) => n.type === 'blockquote' || n.type === 'iframe').length === 0);
  find(tree, (n) => n.type === 'button' && /ig/.test(n.props.className))[0].props.onClick();
  tree = fm.draw();
