...
(async () => {
  ...
  ok('tap → youtube-nocookie iframe (autoplay, fullscreen, NOT lazy — it is made after the tap) + the poster covers the gap + close button + play event', yf && yf.props.src === vid.feedTrailerSrc_('dQw4w9WgXcQ') && yf.props.allow === 'autoplay; encrypted-media; picture-in-picture; fullscreen' && yf.props.allowFullScreen === true && yf.props.loading === undefined && find(tree, (n) => /ff-feed-loading/.test((n.props || {}).className || '')).length === 1 && find(tree, (n) => n.type === 'img').length === 1 && fm.events.join() === 'play:fp1' && /ff-feed-media vid/.test(tree.props.className));
  find(tree, (n) => n.type === 'button')[0].props.onClick();
  ...
  // ⚡ 21 Sep: the half of the old speed PR that still applied after the Instagram embed was removed.
  console.log('--- speed: first load and scrolling');
  ok('the brand font does not hold up the page (preload + media=print swap + a <noscript> copy)', /rel="preload" as="style" href="https:\/\/fonts\.googleapis\.com/.test(html) && /rel="stylesheet" media="print" onload="this\.media='all'/.test(html) && /<noscript><link rel="stylesheet" href="https:\/\/fonts\.googleapis\.com/.test(html) && !/^<link href="https:\/\/fonts\.googleapis[^>]*rel="stylesheet" \/>$/m.test(html));
  ok('the React host is warmed up before it is asked for', /<link rel="preconnect" href="https:\/\/unpkg\.com" crossorigin \/>/.test(html));
  ok('off-screen feed posts are not drawn (with a size hint so scrolling stays honest)', /\.ff-feed-post \{ content-visibility: auto; contain-intrinsic-size: auto 560px;/.test(html));
  ok('the bottom menu has no invisible blur to repaint on every scroll frame', /\.ff-bnav \{[^}]*background: rgba\(255,255,255,\.97\);/.test(html) && !/\.ff-bnav \{[^}]*backdrop-filter/.test(html));
  ok('low-end phones and Data Saver: no header blur, shorter slide', /html\.ff-lite \.ff-topbar, html\.ff-lite \.ff-sheet-bg \{ backdrop-filter: none;/.test(html) && /html\.ff-lite \.ff-slide \{ animation-duration: \.16s; \}/.test(html) && /navigator\.connection && navigator\.connection\.saveData/.test(html) && /saveData\) document\.documentElement\.classList\.add\('ff-lite'\)/.test(html));
  ok('a tapped trailer shows the poster + a spinner until YouTube can be seen', /className: "ff-feed-loading"/.test(html) && /onLoad: \(\) => setTimeout\(\(\) => setReady\(true\), 400\)/.test(html) && /setReady\(false\);/.test(html) && /\.ff-feed-spin \{/.test(html));
  ok('in Reels the poster stays until a Short really starts playing', /!\(on && \(kind === 'video' \|\| \(kind === 'short' && yt === 1\)\)\)/.test(html));
  ok('nothing is preconnected to Instagram any more (nothing is loaded from it)', /const host = kind === 'yt' \? 'https:\/\/www\.youtube-nocookie\.com' : '';/.test(html) && !/instagram/.test((html.match(/function feedPrewarm_[\s\S]{0,400}/) || [''])[0]));
