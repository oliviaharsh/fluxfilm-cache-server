...
(async () => {
  ...
  r = await post('/admin/api/profit/cost', { service: 'Netflix', accountId: 'NF-01', cost: '4800', every: 12, note: 'annual plan' });
  const yi = last(/^INSERT INTO account_costs/);
  ok('yearly cost: stored as ₹400/month + billing tag in note', r.body.ok && r.body.monthlyCost === 400 && r.body.billedEvery === 12 && yi.params[2] === 400 && yi.params[3] === '[billing:12:4800] annual plan', yi.params);
  r = await post('/admin/api/profit/cost', { service: 'Netflix', accountId: 'NF-01', cost: '100', every: 2 });
  ok('billing period must be 1/3/6/12', r.status === 400);
  const { parseBilling } = require('../profit');
  const pb = parseBilling('[billing:3:999] q plan', 333);
  ok('parseBilling reads the tag; plain notes = monthly', pb.every === 3 && pb.amount === 999 && pb.note === 'q plan' && parseBilling('annual / 12', 100).every === 1 && parseBilling('', 100).amount === 100);
  COSTS.push({ service: 'Prime Video', account_id: 'PRI-02', monthly_cost: '83.33', note: '[billing:12:1000] ' });
  r = await get('/admin/api/profit?period=last_month');
  const p2 = r.body.accounts.find((a) => a.accountId === 'PRI-02');
  ok('yearly account: monthly shown, period cost from the exact yearly amount, monthly total', p2.billedEvery === 12 && p2.billedAmount === 1000 && p2.monthlyCost === 83.33 && Math.abs(p2.cost - (1000 / 12) * months) < 0.02 && Math.abs(r.body.totals.monthlyCost - (300 + 100 + 1000 / 12)) < 0.02, p2);
  COSTS.pop();
