...
(async () => {
  ...
  ok('sheet: portal to <body>, keyboard handled with visualViewport (--ff-kb / --ff-vvh), Back button closes (history entry), swipe down / ✕ / Escape', /return ReactDOM\.createPortal\(/.test(html) && /vv\.addEventListener\('resize', upd\)/.test(html) && /setProperty\('--ff-kb'/.test(html) && /\.ff-cs \{[^}]*bottom: var\(--ff-kb, 0px\)/.test(html) && /const layerClose = useFFLayer_\(\(\) => close\(true\)\);/.test(html) && /h\(\)\.pushState\(Object\.assign\(\{\}, h\(\)\.state \|\| \{\}, \{\s*ffLayer: st\.pushed\s*\}\)/.test(html) && /win\.addEventListener\('popstate', onPop\)/.test(html) && /if \(d\.dy > 90\) close\(\)/.test(html) && /"aria-label": "Close comments"/.test(html) && /e\.key === 'Escape'/.test(html));
