...
(async () => {
  ...
  r = await tryOtp('JioHotstar', '9876543210', access.makeToken('9876543210').token);
  ok('old otp1 token (Games / Refund code, profile email) does not unlock Get OTP', r.needsVerify === true, r);
  ok('expired token does not work', !access.verifyToken(access._internal.makeToken2('9876543210', 'a@b.c', Date.now() - 40 * 86400e3).token, '9876543210'));
