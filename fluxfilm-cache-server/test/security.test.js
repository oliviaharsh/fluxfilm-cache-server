...
(async () => {
  ...
  const good = access.makeToken('9876543210').token;
  ...
  subsRows = [{ service: 'JioHotstar' }]; used = 0;
  ...
  r = await tryOtp('JioHotstar', '9876543210', access.makeToken('9000000000').token);
  ok("another number's token does not work", r.needsVerify === true);
  const tamp = good.split('.'); tamp[2] = String(Number(tamp[2]) + 86400e3);
  r = await tryOtp('JioHotstar', '9876543210', tamp.join('.'));
  ok('token with a changed expiry (tampered) does not work', r.needsVerify === true);
  ok('expired token does not work', !access.verifyToken(access.makeToken('9876543210', Date.now() - 40 * 86400e3).token, '9876543210'));
  ok('IMAP not opened without a token', !imapTouched);
  subsRows = [];
  r = await tryOtp('JioHotstar', '9876543210');
  ok('phone with no plan gets no OTP', r.found === false && /active JioHotstar/.test(r.message), r);
  subsRows = [{ service: 'Zee5 Premium' }];
  r = await tryOtp('JioHotstar', '9876543210');
  ok('a plan for a different service does not count', r.found === false && /active JioHotstar/.test(r.message), r);
  subsRows = [{ service: 'JioHotstar' }]; used = 999;
  r = await tryOtp('JioHotstar', '9876543210');
