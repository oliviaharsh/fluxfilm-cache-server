...
(async () => {
  ...
  ok('pop-up after a successful purchase / renewal: once per visit, waits for offer pop-ups, "Not now" = snooze', /if \(screen !== 'done' \|\| !success \|\| !phone\) return;/.test(promptSrc) && /pushAskedThisVisit_\(\) \|\| pushSnoozedUntil_\(\) > Date\.now\(\)/.test(promptSrc) && /if \(window\.ffPromoOpen \|\| window\.ffPushOpen\) return;/.test(promptSrc) && /if \(!manual\) pushSnooze_\(\);/.test(promptSrc));
