...
(async () => {
  ...
  ok('permission asked only from a tap — "Yes, remind me", or 🔔 Notify me on the anniversary bar (never automatically)', (html.match(/Notification\.requestPermission/g) || []).length === 1 && html.slice(start, end).includes('Notification.requestPermission') && (html.match(/pushEnable_\(/g) || []).length === 3 && /const turnOn = \(\) => \{[\s\S]*?pushEnable_\(phone\)/.test(promptSrc) && /onClick: turnOn/.test(promptSrc) && /function press\(\) \{[\s\S]*?pushEnable_\(ph\)\.then/.test(html) && /onClick: press/.test(html));
