...
(async () => {
  ...
  r = await say({ text: 'can i edit profiles in netflix' });
  ok('"can i edit profiles in netflix" is ANSWERED (Private full / Sharing limited), not turned into a Sharing/Private buy pitch', last(r).intent === 'PROFILE_ANSWER' && /Private/.test(last(r).text) && /Sharing/.test(last(r).text) && !ids(last(r)).includes('sharing') && !ids(last(r)).includes('private'), last(r));
  r = await say({ text: 'netflix profile rename kar sakta hu' });
  ok('a profile how-to without a question mark also answered, still not a buy', last(r).intent === 'PROFILE_ANSWER', last(r));
  ...
  await say({ choice: 'menu' });
  r = await say({ text: 'household code chahiye tv ke liye' });
  r = await say({ choice: 'hhcode' });
  ok('retry succeeds once a fresh code exists', last(r).intent === 'HH_CODE_READY' && last(r).card.code === '5150', last(r));
  shop.hhCode = '';
