...
ok('bottom menu self-heal: at the end of the page it measures the last content against the menu / visible screen and adds exactly the missing room', has('var(--ff-bnav-h, 0px)) + 88px + var(--ff-bnav-extra, 0px))') && has('+ 96px + var(--ff-bnav-extra, 0px))') && has('if (doc.scrollTop + visH < doc.scrollHeight - 4) return;') && has('const missing = Math.ceil(last.getBoundingClientRect().bottom - limit + 20);') && has('extra < 480') && has("window.removeEventListener('scroll', onScroll)"));
