...
ok('bottom menu self-heal: at the end of the page it measures the last content against the menu / visible screen and adds exactly the missing room', has('var(--ff-bnav-h, 0px)) + 88px + var(--ff-bnav-extra, 0px))') && has('+ 96px + var(--ff-bnav-extra, 0px))') && has('const layoutH = Math.max(window.innerHeight, doc.clientHeight || 0);') && has('if (doc.scrollTop + layoutH < doc.scrollHeight - 4) return;') && !has('if (doc.scrollTop + visH < doc.scrollHeight - 4) return;') && has("if (pos === 'fixed' || pos === 'absolute' || pos === 'sticky') continue;") && has('ro2.observe(document.body)') && has("window.addEventListener('touchend', onScroll, { passive: true });") && has('const missing = Math.ceil(last.getBoundingClientRect().bottom - limit + 20);') && has('extra < 480') && has("window.removeEventListener('scroll', onScroll)"));
