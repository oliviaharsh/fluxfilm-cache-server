...
ok('active = not refunded and days left >= 0 (0 = expires today), expired = the rest; choice remembered on the phone', /const isLiveSub = s => !s\.refunded && s\.daysLeft != null && s\.daysLeft !== '' && Number\(s\.daysLeft\) >= 0;/.test(html) && /localStorage\.setItem\('ff_sub_filter', v\)/.test(html) && /const filteredSubs = subFilter === 'active' \? liveSubs : subFilter === 'expired' \? expiredSubs : null;/.test(html));
...
{
  // Owner's Nord 5 (~309 CSS px wide): a later ".page-inner{padding:0 12px}" wiped the bottom-menu room (?layout=1: padB 0px).
  const css = html.slice(0, html.indexOf('</style>', html.indexOf('.ff-has-bnav {')) + 2000);
  const after = html.slice(html.indexOf('.ff-has-bnav {'));
  const bad = (after.match(/\.page-inner\s*\{[^}]*\bpadding\s*:[^}]*\}/g) || []);
  ok('no .page-inner rule after the bottom-menu room uses the padding shorthand (it would zero padding-bottom on some widths)', bad.length === 0 && css.length > 0, bad);
  ok('narrow phones keep side padding only', has('@media (max-width:380px){ .page-inner{padding-left:12px;padding-right:12px} }'));
}
...
// Root cause was fixed in PR #74 (padding shorthand guard above); the earlier workarounds are gone for good.
ok('bottom-menu workarounds removed (spacer, standalone +96 px, self-heal, ?layout=1 box)', !has('.ff-has-bnav::after') && !/@media \(display-mode: standalone\) \{ \.ff-has-bnav/.test(html) && !has('--ff-bnav-extra') && !has('ffLayoutDebug') && !has('ff_layout_debug') && !has('ro2.observe(document.body)'));
