...
ok('active = days left >= 0 (0 = expires today), expired = the rest; choice remembered on the phone', /const isLiveSub = s => s\.daysLeft != null && s\.daysLeft !== '' && Number\(s\.daysLeft\) >= 0;/.test(html) && /localStorage\.setItem\('ff_sub_filter', v\)/.test(html) && /const filteredSubs = subFilter === 'active' \? liveSubs : subFilter === 'expired' \? expiredSubs : null;/.test(html));
