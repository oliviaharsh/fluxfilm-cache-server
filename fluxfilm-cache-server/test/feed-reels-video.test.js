...
(async () => {
  ...
  const H = new Function('window', 'navigator', '' + ['feedYouTubeId_', 'feedIgUrl_', 'feedVideoSrc_', 'feedYtShort_', 'feedReelKind_', 'feedShortCrop_', 'feedShortSrc_'].map(fn).join('\n') + '; return { feedReelKind_, feedShortCrop_, feedShortSrc_, feedVideoSrc_ };')({ location: { origin: 'https://shop.fluxfilm.in' } }, {});
  ...
  ok('crop CSS: overflow hidden box, header / footer outside; our tap layer covers a Short once YouTube reports it started (before that its own play button can be tapped)', /\.ff-reel-crop \{ position: absolute; inset: 0; z-index: 3; overflow: hidden;/.test(html) && /\.ff-reel-crop\.short \{ z-index: 1; \}/.test(html) && /i === idx && playing && \(kind === 'short' \|\| kind === 'yt'\) && \(yt === 1 \|\| yt === 2 \|\| yt === 3\) && React\.createElement\("div", \{\s*className: "ff-reel-tap"/.test(html) && /style: px\(feedShortCrop_\(dims\.w, dims\.h\)\)/.test(html));
  // 📸 16 Sep → 18 Sep: Instagram's /embed/ page paints its own "View more on Instagram" bar ON TOP of the media,
  // at a height that depends on that post's poster, so no crop can hide it. The embed is gone: we show OUR poster
  // and open Instagram on a tap (the owner's screenshot of a white stripe across a Netflix reel).
  ok('no Instagram embed anywhere: no embed.js, no /embed/ iframe, no crop maths', !/instagram\.com\/embed\.js|feedIgMount_|feedIgScript_|feedIgCrop_|instagram-media|FEED_IG_HEADER_/.test(html) && !/\+ 'embed\/'/.test(html));
  ok('Instagram post in Reels: our poster + "Watch on Instagram" (and "Watch the trailer here" when the post has one); a tap opens Instagram', /kind === 'ig' && React\.createElement\("div", \{\s*className: "ff-reel-igfb"/.test(html) && /"📸 Watch on Instagram ↗"/.test(html) && /"▶ Watch the trailer here"/.test(html) && /if \(kind === 'ig'\) \{\s*window\.open\(feedIgUrl_\(c\.p\.instagramUrl\), '_blank', 'noopener'\);/.test(html) && /'Tap to watch it on Instagram'/.test(html) && !/ig-on/.test(html));
  ok('Instagram post in the feed: the play button is a link to Instagram, no inline player', /ig \? React\.createElement\("a", \{\s*href: ig,\s*target: "_blank"/.test(html) && /"📸 Watch on Instagram ↗"\)\) : yt/.test(html) && !/play === 'ig'/.test(html) && !/ff-feed-igbox/.test(html));
  ...
  ok('one video at a time: only the reel on screen renders a <video> / Short / trailer, every start goes through feedVideoStart_', /i === idx && playing && kind === 'video' && React\.createElement\("video"/.test(html) && /i === idx && playing && kind === 'short' && dims\.h > 0/.test(html) && /i === idx && playing && kind === 'yt' && React\.createElement\("iframe"/.test(html) && /feedVideoStart_\('reels', \(\) => \{/.test(html) && /function syncIdx\(\) \{[\s\S]{0,400}setIdx\(i\);[\s\S]{0,200}stop\(\);/.test(html));
  ok('our player: autoplay, loop, playsinline, progress bar, 🔊 sound button, tap pauses', /autoPlay: true,\s*loop: true,\s*playsInline: true/.test(html) && /el\.setAttribute\('muted', ''\)/.test(html) && /className: "ff-reel-prog"/.test(html) && /barRef\.current\.style\.transform = 'scaleX\('/.test(html) && /className: "ff-reel-mute"/.test(html) && /if \(v\.paused\) v\.play\(\)\.catch\(\(\) => \{\}\);else v\.pause\(\);/.test(html));
  // 🔊 Reels open with the sound on (owner, 16 Sep): muted only if the phone refuses to autoplay with sound.
  ok('Reels start with the sound ON, the choice is remembered for the visit, and a refused unmuted autoplay falls back to 🔇', /const feedReelSound_ = \{\s*muted: false\s*\};/.test(html) && /const \[muted, setMutedState\] = useState\(feedReelSound_\.muted\);/.test(html) && /feedReelSound_\.muted = on;\s*setMutedState\(on\);/.test(html) && /const mutedRef = useRef\(feedReelSound_\.muted\);/.test(html) && /el\.muted = mutedRef\.current;/.test(html) && /pr\.catch\(\(\) => \{\s*el\.muted = true;\s*setMuted\(true\);/.test(html) && !/const \[muted, setMuted\] = useState\(true\)/.test(html));
  // 🎬 Drag the bottom line to move the reel (Instagram).
  ok('the bottom line is draggable: bigger touch strip, knob + m:ss while dragging, seeks the video, resumes after, never scrolls the list or pauses by tap', /className: "ff-reel-prog" \+ \(scrub \? \' drag\' : \'\'\),\s*ref: scrubRef,\s*role: "slider"/.test(html) && /onPointerDown: scrubStart,\s*onPointerMove: scrubMove,\s*onPointerUp: scrubEnd,\s*onPointerCancel: scrubEnd,\s*onClick: e => e\.stopPropagation\(\)/.test(html) && /v\.currentTime = frac \* v\.duration;/.test(html) && /setPointerCapture\(e\.pointerId\)/.test(html) && /on && !scrub && \(kind === 'video' && paused/.test(html) && /if \(v && !wasPaused\) v\.play\(\)\.catch/.test(html) && /if \(!dragRef\.current && v\.duration\) paintBar\(/.test(html) && /\.ff-reel-prog \{[^}]*touch-action: none;/.test(html) && /\.ff-reel-prog\.drag \.ff-reel-track \{ height: 5px; \}/.test(html) && /className: "ff-reel-knob"/.test(html) && /className: "ff-reel-time"/.test(html));
  const CLK = new Function(fn('feedClock_') + '; return feedClock_;')();
  ok('m:ss clock', CLK(0) === '0:00' && CLK(7) === '0:07' && CLK(67.8) === '1:07' && CLK(605) === '10:05' && CLK(NaN) === '0:00' && CLK(-5) === '0:00');
