...
(async () => {
  ...
  ok('Instagram iframe: no popups (profile links go nowhere), /embed/ only, no Instagram bar of ours', /sandbox: "allow-scripts allow-same-origin allow-presentation"/.test(html) && /feedIgUrl_\(p\.instagramUrl\) \+ 'embed\/'/.test(html) && !/ff-reel-igbar/.test(html));
