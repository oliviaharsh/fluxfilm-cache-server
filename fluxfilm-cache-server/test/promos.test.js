...
(async () => {
  ...
  ok('checkout: new-order coupon pre-filled from the offer and applied once; renewal pre-filled', /coupon: getPromoCoupon_\(\),/.test(html) && /promoApplied\.current = true;\s*applyCoupon\(\);/.test(html) && /const \[couponCode, setCouponCode\] = useState\(\(\) => renewCoupon \|\| getPromoCoupon_\(\)\);/.test(html)); // renewCoupon: renewal reminder pop-up
  ...
  ok('install pop-up waits while an offer pop-up is open', /if \(window\.ffPromoOpen \|\| window\.ffRenewPopOpen\) return;/.test(html));
