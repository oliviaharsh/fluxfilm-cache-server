...
(async () => {
  ...
  ok('credit email: amount due + due date, valid-till date', /Amount due: ₹149/.test(emC.html) && /by 19 Sep 2026/.test(emC.html) && /valid till 1 Oct 2026/.test(emC.html) && /₹149 payment due/.test(emC.subject), emC);
  ...
  ok('WhatsApp expired text', wt === 'Hi Mohammad, your FluxFilm Prime Video (1 Month) expired on 1 Sep 2026. Renew here: https://shop.fluxfilm.in/?source=push&renew=SUB-SOURAB 🙏', wt);
  ...
  ok('WhatsApp credit text includes "₹149 is due"', /^Hi Asha, your FluxFilm Netflix \(Private 1M\) is renewed till 1 Oct 2026\. ₹149 is due by 19 Sep 2026/.test(wc), wc);
  ...
  const cardFn = new Function('return (function(){ var S = { cust: { profile: { name: "Mohammad Sourab" } } }; var MON = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]; function esc(v){return String(v==null?"":v);} function money(n){return "₹"+n;} function statusPill(){return "";} function svcIcon(){return "";} function waLink(p,t){return "wa:"+encodeURIComponent(t);} ' + grab('prettyDate') + grab('istDate') + grab('daysLeft') + grab('crRenewLink') + grab('crFirst') + grab('crWaText') + grab('crDuePill') + grab('crExtrasInner') + grab('crSubExtras') + grab('subCard') + ' return subCard; })()')();
  ...
  ok('expired card: ✉️ Email reminder + 💬 text "Hi Mohammad, … expired on 1 Sep 2026. Renew here: …renew=SUB-SOURAB"', /crRemindDialog\('SUB-SOURAB'\)/.test(expiredCard) && expiredCard.includes(encodeURIComponent('Hi Mohammad, your FluxFilm Prime Video (1 Month) expired on 1 Sep 2026. Renew here: https://shop.fluxfilm.in/?source=push&renew=SUB-SOURAB')), expiredCard.slice(-900));
  ...
  const alone = new Function('return (function(){ var MON = ["Jan"]; function esc(v){return String(v==null?"":v);} function statusPill(){return "";} function svcIcon(){return "";} function waLink(){return "#";} function prettyDate(v){return String(v||"");} ' + grab('istDate') + grab('daysLeft') + grab('subCard') + ' return subCard; })()')();
  ok('subCard alone (without the credit block) still renders', /⏱ Extend/.test(alone({ sub_id: 'S', status: 'ACTIVE', expiry_date: '2026-09-01 10:00:00' })));
