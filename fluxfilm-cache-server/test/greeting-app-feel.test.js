...
r = g({ name: 'Yahya', now: wed(12), lastSeen: null, isNew: true });
ok('brand new customer (no plans, no orders) → welcome line', r.key === 'first', r);
r = g({ name: 'Yahya', now: wed(12), lastSeen: null, isNew: false, subs: [{ service: 'Netflix', daysLeft: 20 }] });
ok('missing last-seen on this phone but has orders/plans → NOT "new", normal time-of-day line', r.key === 'afternoon' && !/Welcome to the club|binge era starts/.test(r.text), r);
r = g({ name: 'Yahya', now: wed(12), lastSeen: null });
ok('missing last-seen and unknown history → never "new"', r.key !== 'first', r);

section('India calendar days (review fixes)');
// 09:10 IST on Wed 16 Sep; plan expires 18:00 IST the same day (server sends ISO UTC; old Math.ceil gave daysLeft=1).
const nine = wed(9);
r = g({ name: 'Yahya', now: nine, lastSeen: recent(nine), subs: [{ service: 'Netflix', daysLeft: 1, expiryDate: '2026-09-16T12:30:00.000Z' }] });
ok('expires today 18:00, seen 09:00 → "ends today" (not tomorrow)', r.key === 'ending0' && /ends today/.test(r.text) && !/tomorrow/.test(r.text), r);
r = g({ name: 'Yahya', now: nine, lastSeen: recent(nine), subs: [{ service: 'Netflix', daysLeft: 1, expiryDate: '2026-09-16 18:00:00' }] });
ok('Safari-unsafe "YYYY-MM-DD HH:mm:ss" read as India time → today', r.key === 'ending0', r);
r = g({ name: 'Yahya', now: wed(23), lastSeen: recent(wed(23)), subs: [{ service: 'Netflix', daysLeft: 1, expiryDate: '2026-09-17T03:30:00.000Z' }] });
ok('23:10 tonight, expires 09:00 tomorrow → "tomorrow"', r.key === 'ending1' && /tomorrow/.test(r.text), r);
r = g({ name: 'Yahya', now: nine, lastSeen: recent(nine), subs: [{ service: 'Netflix', daysLeft: 3, expiryDate: '2026-09-19T12:30:00.000Z' }] });
ok('3 calendar days → "in 3 days"', r.key === 'ending' && /\b3\b/.test(r.text) && !/today|tomorrow/.test(r.text), r);
r = g({ name: 'Yahya', now: nine, lastSeen: recent(nine), subs: [{ service: 'Netflix', daysLeft: 0, expiryDate: '2026-09-16T00:30:00.000Z' }] });
ok('expired earlier today → not an "ends today" reminder', !/^ending/.test(r.key), r);
r = g({ name: 'A', now: new Date(Date.UTC(2026, 8, 16, 20, 0)), lastSeen: Date.UTC(2026, 8, 16, 19, 0) });
ok('time of day uses India hour (20:00 UTC → 01:30 India → late night)', r.key === 'lateNight', r);

section('all plans expired');
const expSubs = [{ service: 'Netflix', daysLeft: -4, expiryDate: '2026-09-12T12:30:00.000Z' }, { service: 'Prime Video', daysLeft: -20 }];
const expired = Array.from({ length: 6 }, (_, k) => g({ name: 'Yahya', now: wed(9), lastSeen: recent(wed(9)), subs: expSubs, skip: k }));
ok('all expired → gentle renew nudge, never "Plans are all good"', expired.every((x) => x.key === 'expired' && !/all good/i.test(x.text)), expired);
const noPlanMorning = Array.from({ length: 6 }, (_, k) => g({ name: 'Yahya', now: wed(9), lastSeen: recent(wed(9)), subs: [], skip: k }).text);
ok('no active plan → morning lines skip "Plans are all good"', noPlanMorning.every((t) => !/all good/i.test(t)), noPlanMorning);
const liveMorning = Array.from({ length: 6 }, (_, k) => g({ name: 'Yahya', now: wed(9), lastSeen: recent(wed(9)), subs: [{ service: 'Netflix', daysLeft: 20 }], skip: k }).text);
ok('active plan → "Plans are all good" can show', liveMorning.some((t) => /all good/i.test(t)), liveMorning);
ok('away line is playful, not "Rude"', !W.FF_GREETING_LINES.away.some((l) => /Rude/.test(l[0])) && W.FF_GREETING_LINES.away.some((l) => /I counted/.test(l[0])));
...
r = g({ name: 'Yahya', now: new Date(2026, 10, 8, 20, 0), lastSeen: null, isNew: true });
...
ok('names: first word, capitalised; empty / "User" → friend; markup stripped; long names cut', g({ name: 'yahya khan', now: wed(9), lastSeen: null, isNew: true }).text.includes('Yahya') && g({ name: '', sassy: false }).text === 'Welcome back, friend' && g({ name: 'User', sassy: false }).text === 'Welcome back, friend' && g({ name: '<b>Ravi', sassy: false }).text === 'Welcome back, BRavi' && g({ name: 'Abcdefghijklmnopqrst', sassy: false }).text === 'Welcome back, Abcdefghijklmn');
...
ok('avatar button opens Account with Enter / Space too', /className: "ff-hello-av",[\s\S]{0,400}onKeyDown: e => \{\s*if \(e\.key !== 'Enter' && e\.key !== ' ' && e\.key !== 'Spacebar'\) return;\s*e\.preventDefault\(\);\s*nav\('account'/.test(dash));
ok('"new customer" decided from data (no plans, orders loaded and empty), not phone storage', /const helloIsNew = !!dashData\?\._checked && allSubs\.length === 0 && !!helloPhone && ordersPhone === helloPhone && !ordersLoading && orders\.length === 0;/.test(dash));
...
(async () => {
  ...
  ok('admin turns it ON → change log "Sassy greeting … ON" (not "maintenance")', x.body.ok && audits.length === 1 && audits[0].action === 'store.greeting' && /Sassy greeting on My plans turned ON/.test(audits[0].summary), audits);
  ...
  audits.length = 0;
  x = await call('POST', '/admin/api/store', { sassyGreeting: false, helpBubble: false, message: 'Back soon' });
  ok('greeting + help bubble + message in one save → three separate, correct log lines', x.body.ok && audits.length === 3 && audits.some((a) => a.action === 'store.greeting' && /OFF/.test(a.summary)) && audits.some((a) => a.action === 'store.helpbubble' && /OFF/.test(a.summary)) && audits.some((a) => /Updated maintenance message/.test(a.summary)), audits);
  audits.length = 0;
  x = await call('POST', '/admin/api/store', { helpBubble: true });
  ok('help bubble alone → only the help bubble log (not greeting / maintenance)', audits.length === 1 && audits[0].action === 'store.helpbubble', audits);
  const st2 = await store.getStatus();
  ok('public status carries BOTH switches', st2.helpBubble === true && st2.sassy === false, st2);
