...
(async () => {
  ...
  // The "from ₹X" is now dropped when the whole platform is sold out (prices-in-stock.test.js), so the CTA
  // reads 'Get ' + info.service + (info.price ? ' from ₹' + info.price : '') and the price carries the stock.
  ok('post header uses brandInfo (name + logo), CTA still sells info.service with "Streaming on <brand>"', /brandInfo: feedBrandInfo_\(feedPostBrand_\(p\), plans\)/.test(html) && /const name = bi\.name;/.test(html) && /'Get ' \+ info\.service \+ \(info\.price \? ' from ₹' \+ info\.price : ''\)/.test(html) && /'Streaming on ' \+ name/.test(html) && /const info = p\.cta === 'service' \? feedServiceInfo_\(plan, plans, shareText_\.data\.levels\) : null;/.test(html));
