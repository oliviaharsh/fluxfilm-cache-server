 ...
 *     ai, provider, angle, grounded, rewrite, tmdb, tokens, notes[], message }
 *
...
async function aiFill(input, deps) {
  ...
  let ai = false; let tokens = 0; let tmdb = false; let provider = ''; let angle = ''; let grounded = false;
  ...
  return {
    ok: true, fields, ai, provider, angle, grounded, rewrite, tmdb, tokens, notes,
    message: got.length ? '✨ Filled ' + got.join(', ') + (provider ? ' (' + provider + (grounded ? ' + Google Search' : '') + ')' : '') + ' — check and 💾 Save' : 'Nothing found to fill.',
  };
}

module.exports = { aiFill, GENRES, mapGenres, cleanCaption, CAPTION_MAX, CAPTION_ASK, ANGLES, GENERIC, EXAMPLES, _internal: { prompt, factLines, needsSearch, models, isIndian } };
